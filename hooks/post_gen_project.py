...
# Flags (ensure these exist in cookiecutter.json)
...
def rm(relpath: str) -> None:
    ...
    if p.is_dir():
        ...
        try:
            ...
            # Make writable then retry (Windows)
...
if not USE_PY:
    rm("pyproject.toml")
    rm(".python-version")
    # If you ship Python-only configs later, prune them here:
    # rm("mypy.ini"); rm("ruff.toml"); rm("pytest.ini"); rm("tests")

if not USE_NODE:
    rm("package.json"); rm("package-lock.json"); rm("pnpm-lock.yaml"); rm("yarn.lock")
...
if not USE_RS:
    ...
    rm("src/main.rs"); rm("src/lib.rs"); rm("tests/sanity.rs")

# 3) Docs
if not USE_MKDOCS:
    rm("mkdocs.yml"); rm("docs")
...
if INIT_GIT:
    try:
        ...
        # Set default branch to main if supported
